...
def criar_tela_inicio(root):
    ...
    # SCROLL VIEW
    # ============================================================
    canvas = ctk.CTkCanvas(container, bg=FUNDO, highlightthickness=0)
    ...
    # Frame que ficará dentro do scroll
    scroll_frame = ctk.CTkFrame(canvas, fg_color=FUNDO)
    ...
    # Frame centralizador
    content = ctk.CTkFrame(scroll_frame, fg_color=FUNDO)
    content.pack(expand=True)

    # Atualiza área do scroll
    def atualizar_scroll(event):
        canvas.configure(scrollregion=canvas.bbox("all"))

        # Faz o frame ocupar toda largura da tela
    ...
    # --- NAVBAR ---
    navbar = ctk.CTkFrame(content, fg_color=BRANCO, height=80, corner_radius=0)
    navbar.pack(fill="x")
    ...
    ctk.CTkLabel(content, text="Bem-Vindo ao PetFinder",
                 text_color=CINZA, font=("Segoe UI", 38, "bold")).pack(pady=(40, 10))

    ctk.CTkLabel(content, text="Ajude a reunir famílias com seus pets.",
                 text_color=SUBTITULO, font=("Segoe UI", 18)).pack()

    # --- CARDS DE AÇÃO  ---
    cards_frame = ctk.CTkFrame(content, fg_color=FUNDO)
    ...
    ctk.CTkLabel(content, text="Animais Recentes",
                 text_color=CINZA, font=("Segoe UI", 30, "bold")).pack(pady=(20, 30))

    pets_frame = ctk.CTkFrame(content, fg_color=FUNDO)
    pets_frame.pack()
    ...
    for pet in pets:
        ...
        card = ctk.CTkFrame(pets_frame, fg_color=BRANCO,
                            corner_radius=25, width=260, height=290)
        ...
        ctk.CTkButton(
            ...
            command=lambda p=pet: abrir_detalhes(root, p)
